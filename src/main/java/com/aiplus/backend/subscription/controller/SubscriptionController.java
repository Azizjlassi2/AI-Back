...
public class SubscriptionController {
    ...
    public ResponseEntity<ApiResponse<String>> createSubscription(@RequestBody SubscriptionCreateDTO dto) {
        String payUrl = subscriptionService.createSubscription(dto);
        return ResponseEntity.ok(ResponseUtil.success("Subscription created", payUrl));
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<Subscription>> getSubscription(@PathVariable Long id) {
        return subscriptionService.getSubscriptionById(id)
                .map(subscription -> ResponseEntity.ok(ResponseUtil.success("Subscription retrieved", subscription)))
